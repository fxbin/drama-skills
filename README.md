...
打开是一张项目清单：每部剧有几集、写到哪一步、已经攒下哪些图片视频音频。
点进一集，这一集的文件按创作顺序排开——剧本、分镜、提示词，正文直接读，
`.jsonl` 这类结构化产物换成中文标签（分集安排、人物设定、关键道具），不必对着记录名认。
图片和视频在页面里就能看，剧本和梗概能就地改一段，保存前会核对文件有没有被别处动过。

它只做看和改。出图出片、审查、导出交付都从各自的 skill 发起，创作台不配置供应商、不提交生成任务。

**只能在 macOS 与 Linux 上启动。**它把读写锁在项目目录里，靠的是 `O_NOFOLLOW` 和目录文件
描述符，Windows 没有这两样，所以创作台会直接拒绝启动——套件安装和命令行的项目工具在
Windows 上照常可用。
