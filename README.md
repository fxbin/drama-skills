...
产出是文本：剧本、设定、提示词、审查记录；套件不调用真正的图片、视频或音频生成服务。

## 演示

《孤身入魔》演示工程取材自成熟古装玄幻项目，包含项目设定、两集剧本、十二板分镜、
宣发图，以及一条 15 秒竖屏成片——连续长镜头，顾霖背棺赴关。
...
技能会选好项目、分配回环端口并打开浏览器。控制台用于浏览和轻量编辑项目文本、
只读预览图片与视频、查看项目状态。只跑在本机，需要 macOS 或 Linux。

## 设计取舍

三条原则贯穿整条链路：剧本交付**可表演、可制作的事实**；资产拥有**身份与状态**，
分镜只拥有**本镜呈现**；连续性**明确记录**，精确比较相邻镜头的已确认边界。

规则分四级——可机器校验的结构要求、需结合证据判断的内容要求、通常有帮助的做法，
以及创作者自选的风格。确定性脚本只守第一级：文件、引用、状态与交付完整性。
创作判断始终保留弹性。
