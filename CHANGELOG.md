...
- **本地项目 Dashboard**：新增零生产依赖的 Python 本地服务和三栏项目工作台，按项目文本、
  宣发素材与参考输入浏览 `short-drama.json` 工程；支持 UTF-8 文本编辑、SHA-256 乐观并发、
...
- **中文项目目录**：新项目使用 `输入/`、`项目开发/`、`设定集/`、`剧集/`、`交付/`、
  `创作者决策/`、`审查/` 与 `宣发/`。现有英文目录继续支持，打包与校验沿用项目当前布局。
...
- **共享文件的失效半径按路径而非记录**：`设定集/*.jsonl` 与 `short-drama.json` 是全项目
  共享输入，任何增补会把引用它们的全部下游产物标记 `stale`。文档承诺的「只标记依赖该
  ID/variant 的产物」目前实现不到。规避做法是按弧线拆分设定集并规划资产增补窗口。
