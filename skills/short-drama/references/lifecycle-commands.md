...
服务端分别解析，任一层发现格式错误都拒绝替换原文件。媒体区把文件名含
`fallback` / `preview` / `previs` 的视频标为本地预演，避免把占位视频当成正式成片。
项目内容读写要求运行平台支持安全目录文件描述符（macOS/Linux）；不支持时显式拒绝，
不使用存在符号链接竞态的降级路径。
...
`审查`。旧版英文目录继续兼容。目录名写错会建出平行目录树，`status` 无法归入标准阶段。
