...
`分镜.md` 的「输入参考图」路径只是创作阶段的可读依据与使用意图，不是生产输入快照。进入生产时，必须把本次实际文件逐一写入 job `references`；`prepare` 展示的 `references` 与已确认 job 才是本次 adapter 实际读取哪些文件字节的权威。新生产结果不自动回填或刷新分镜；需要把它改为后续输入时，由分镜 owner 修订文档，再建立新 job 并重新预览、确认。
