...
| 子镜密度 | 一镜就是一段不间断的生成片段，9:16 信息流上的注意力是按切镜买的：可看的切镜密度大致每 4–8 秒一次。一场平均每镜超过十秒时，无论单帧多好，观众读到的都是幻灯片。爽点更紧、情感处理更松是在这条带宽内浮动，每次切换仍要有注意、压力或状态理由。 |
| 单元时长 | 镜头时长从项目已接受的可生成长度集合里取一档，不是自由数字（集合内容由项目声明）。先从必须保留的动作、反应、对白与最终状态反推需要多久，再取集合中容得下它的最短一档；放不下就删减、拆镜或请求改边界。超过约五到八秒后，一张冻结关键帧只描述了镜头开头，其余部分实际上没有规格：更长的镜头要么按精确状态边界拆开，要么由创作者明确接受它是一个宽规格镜头。 |
