...
- 本词表全部是 `taste_option`：它是供比较的表达选项，不是质量门槛。某个方向被创作者接受并
...
- 不用量、词数或“必须包含某词”设门槛。项目级基底选一套；镜头级词只取能解释当前叙事
  职责或可见风险的部分。
...
画风基底属于项目，不属于某一种镜头。先比较一套可观察的视觉投影，接受后把“选择 + 可观察
写法”一起记录到 `visual_direction.choices.look_development`；叙事、对话和动作帧都从同一条
记录读取。
...
用于以台词和神态为主的镜头。漫剧剪辑中这类镜头占比高，风险集中在面部、目光、持物与
背景竞争。
...
2. 读取已接受的 `visual_direction.choices.look_development`。若画风仍只有名称，或本轮选词尚未
   被记录，先回到 `$short-drama` 完成 Look Development、发布并接受创作者决策，再用
   `set-authority` 写回；不要在第七集重新挑一遍。
...
1. **项目级画风打架**：同一项目并写“赛璐璐动画帧”和“厚涂写实”，却没有创作者接受的
   局部例外。先回到视觉方向做选择，不在单镜里临时折中。
...
5. **把镜头类型写进正文**：“叙事关键帧、对话镜头”是选词入口，不是画面内容；正文不写
   分类名、编号与状态词（`SHT-21`）。
6. **选完不记录**：第一集选“国风条漫”，第七集重新回表挑“厚涂漫画”。项目级选择应写入
   `visual_direction.choices.look_development`，后续镜头读取同一条已接受记录。
7. **风格名称没有可观察投影**：只写“国风、电影感、氛围感”，却没有说明线条、色层、
   阴影边缘、材质或空间怎样变化。名称是入口，不是成品。
8. **词表当质量门槛**：审查时只因“没用词表词汇”打回。词汇被创作者接受进视觉方向之前，
   它只是 `taste_option`。
