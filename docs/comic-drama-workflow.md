...
关键顺序有三条：**资产先于分镜**（镜头绑定已接受的人物 / 场景 / 道具版本），
**确认先于生产**（图片 / 视频任务先展示准确预览，再由创作者明确确认），以及
**审查先于文本交付**（只有当前且已批准的文本 / JSON 才能进入交付包）。其余步骤可按项目
...
风格只投影第 2 步已接受的视觉方向；分辨率、供应商控制词与固定质量词尾留给制作配置，
不写进通用资产提示词。
...
- **提示词与供应商解耦**：不同生成服务支持的控制语法、分辨率和风格预设并不相同；
  通用提示词 + 生产环节的可选 adapter，让项目文件在切换服务时不必整体重写。
- **结构化规格先于渲染文本**：规格（JSONL）是唯一事实来源，Markdown 提示词是派生文本。
  直接手写提示词的项目，改资产时要靠记忆同步多处文本；规格化的项目改一处、重新渲染。
- **确认门禁在生产之前**：图片 / 视频生成会消耗外部资源，且结果不可预测。“先预览准确
  任务、明确确认、再执行”把返工挡在调用之前，也让每次生成都可归属到某次确认。
- **审查在生产与交付之间**：审查既能核对文本事实，也能在有生产观察时定位实际失效点；
  交付包只接收当前且已批准的结果，避免把已过期或待修订版本包装成最终稿。
- **事实先于视觉语言**：画风词不能说明角色是谁、站在哪里或手里拿着什么。先写身份、空间与
  连续性事实，再投影已接受的视觉方向，才能同时保持可剪辑性与跨集画风稳定。
